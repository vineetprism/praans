...
const API_BASE = (process.env.NEXT_PUBLIC_API_BASE || "https://prns.prisminfoways.com").replace(/\/+$/, "");
const STATES_API_BASE = (process.env.NEXT_PUBLIC_STATES_API_BASE || "https://prns.prisminfoways.com").replace(/\/+$/, "");
...
async function getStatesData() {  
  try {
    const res = await fetch(`${STATES_API_BASE}/api/states`, {
      next: { revalidate: 1800 }, // align caching
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}: Failed to fetch states data`);
    return await res.json();
  } catch (error) {
...
// Main server component — await searchParams (App Router async API)
