...
//   // Derive states list (from server data + current page data)
//   const states = useMemo(() => {
//     const s = new Set<string>();
//     s.add("All States");
    
//     // Add states from server if available
//     availableStates.forEach(state => s.add(state));
    
//     // Also add states from current page data as fallback
//     rows.forEach(r => r.state_name && s.add(r.state_name));
    
//     return Array.from(s);
//   }, [rows, availableStates]);
...
//     setStateFilter(value === "All States" ? "" : value);
//     if (currentPage !== 1) handlePageChange(1);
//   };
...
// // GazetteNotificationsClient.tsx
...
//     // Add states from server if available
//     availableStates.forEach(state => s.add(state));
//     console.log("availableStates:", availableStates); // Check if availableStates is passed properly
...
//                   <SelectContent>
//   {states.map((state) => (
//     <SelectItem key={state.name} value={state.name} className="text-xs sm:text-sm">
//       {state.name}
//     </SelectItem>
//   ))}
// </SelectContent>
