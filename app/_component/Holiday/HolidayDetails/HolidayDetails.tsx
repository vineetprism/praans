...
}: { selected: Date | null; onChange: (v: Date | null) => void; placeholder?: string }) {
  return (
    <input
      type="date"
...
export default function HolidayDetails({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const res = await fetch(`${API_BASE}/api/holidaysdetail/${slug}`, { signal: ac.signal });
  ...
  const filteredHolidays = useMemo(() => {
    return holidayDetails.filter((h) => {
      const monthOk = selectedMonth === "all" || h.month.toLowerCase() === selectedMonth.toLowerCase();
  ...
  const clearDateFilters = () => { setStartDate(null); setEndDate(null); };
                      ...
                      <div className="flex flex-col min-[480px]:flex-row gap-2 min-[480px]:gap-1 sm:gap-2">
                        <Select value={selectedMonth} onValueChange={setSelectedMonth}>
                          ...
                          <SelectContent>
                            ...
                            {months.map((m) => (
                              <SelectItem key={m} value={m} className="text-xs sm:text-sm">{m}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (apiData.holiday_pdf_url) window.open(apiData.holiday_pdf_url, "_blank");
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4
                  max-w-fit mx-auto place-items-center">
      ...
      <DatePicker selected={endDate} onChange={setEndDate} placeholder="End date" />
            ...
            <div className="block sm:hidden space-y-2 min-[375px]:space-y-3">
              {filteredHolidays.map((h, i) => (
                <div key={`${h.holiday_name}-${i}`} className="bg-white rounded-lg border border-gray-200 p-3 shadow-sm hover:border-orange-200 transition-colors">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 pr-2">
                      <h3 className="font-semibold text-xs sm:text-sm text-gray-900 line-clamp-2 mb-1">{h.holiday_name}</h3>
                    ...
                    <div className="flex-shrink-0">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        h.type === "National" ? "bg-green-100 text-green-800"
                        : h.type === "Regional" ? "bg-blue-100 text-blue-800"
            ...
            <div className="hidden sm:block md:hidden space-y-3">
              {filteredHolidays.map((h, i) => (
                <div key={`${h.holiday_name}-${i}`} className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm hover:border-orange-200 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 flex-1">
                      <span className="w-8 h-8 bg-orange-400 rounded-full text-white text-sm font-medium flex items-center justify-center">{i + 1}</span>
                      <div className="flex-1">
                        <h3 className="font-semibold text-sm text-gray-900 mb-1">{h.holiday_name}</h3>
                        <div className="text-sm text-gray-600">
                          {formatDisplayDate(h.date)} • {h.day} • {h.month.slice(0, 3)}
                        </div>
                      </div>
                    </div>
                    <div className="flex-shrink-0">
            ...
            <div className="hidden md:block bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden lg:max-w-6xl xl:max-w-7xl 2xl:max-w-[1400px] min-[1600px]:max-w-[1560px] min-[1800px]:max-w-[1720px] min-[1920px]:max-w-[1880px] lg:mx-auto">
              <div className="overflow-x-auto">
                <table className="w-full table-fixed">
                  <thead>
                    <tr className="bg-orange-500 border-b border-gray-200">
                      {["S.No", "Holiday Name", "Month", "Date", "Day", "Type"].map((h) => (
                        <th key={h} className="text-left font-semibold text-white whitespace-nowrap p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4 text-xs md:text-sm lg:text-sm xl:text-[13px] 2xl:text-[15px] min-[1600px]:text-base">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {filteredHolidays.map((h, i) => (
                      <tr key={`${h.holiday_name}-${i}`} className="border-b border-gray-100 hover:bg-orange-50 transition-colors">
                        <td className="p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4 text-gray-700 text-xs md:text-sm lg:text-sm xl:text-[13px] 2xl:text-[15px] min-[1600px]:text-base">{i + 1}</td>
                        <td className="p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4 text-gray-700 text-xs md:text-sm lg:text-sm xl:text-[13px] 2xl:text-[15px] min-[1600px]:text-base truncate">{h.holiday_name}</td>
                        <td className="p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4 text-gray-700 text-xs md:text-sm lg:text-sm xl:text-[13px] 2xl:text-[15px] min-[1600px]:text-base">{h.month.slice(0, 3)}</td>
                        <td className="p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4 text-gray-700 text-xs md:text-sm lg:text-sm xl:text-[13px] 2xl:text-[15px] min-[1600px]:text-base">{formatDisplayDate(h.date)}</td>
                        <td className="p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4 text-gray-700 text-xs md:text-sm lg:text-sm xl:text-[13px] 2xl:text-[15px] min-[1600px]:text-base">{h.day}</td>
                        <td className="p-2 md:p-3 lg:p-2 xl:p-2 2xl:p-3 min-[1600px]:p-4">
                          <span className={`rounded-full font-medium px-2 py-1 md:px-3 md:py-1 lg:px-2 lg:py-1 2xl:px-3 2xl:py-1 min-[1600px]:px-4 min-[1600px]:py-1.5 text-xs md:text-sm lg:text-[12px] xl:text-[11px] 2xl:text-sm min-[1600px]:text-[15px] ${
                            h.type === "National" ? "bg-green-100 text-green-800"
                            : h.type === "Regional" ? "bg-blue-100 text-blue-800"
                            : "bg-gray-100 text-gray-800"
                          }`}>{h.type}</span>
