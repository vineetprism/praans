...
//   maximum_rate: number | null;
//   applicable_states: ApplicableState[];
//   non_applicable_states: (string | null | undefined)[]; // can contain non-strings
// };
...
//     // From applicable states (objects)
//     applicableStates.forEach((s) => {
//       if (s?.state_name) all.push(s.state_name);
//     });

//     // From non-applicable states (strings)
//     nonApplicableStates.forEach((n) => {
//       if (typeof n === "string") all.push(n);
//     });

//     return Array.from(new Set(all)) // deduplicate
//       .filter((s) => typeof s === "string")
//       .sort((a, b) => a.localeCompare(b)) // safe sort
//       .map((s) => ({ label: s, value: s }));
//   }, [applicableStates, nonApplicableStates]);

//   // ✅ filtering (with safety guards)
//   const filteredApplicable = useMemo(() => {
//     const term = query.trim().toLowerCase();
//     return applicableStates.filter((s) => {
//       const byText = s?.state_name?.toLowerCase().includes(term);
...
type PTApi = {
  ...
  maximum_rate: number | null;
  applicable_states: ApplicableState[];
  non_applicable_states: ApplicableState[]; // 👈 FIXED (was string[])
};
