"use client";

import React from "react";

type Benefit = {
    title: string;
    desc: string;
};

const BENEFITS: Benefit[] = [
    {
        title: "Legal Compliance",
        desc:
            "Ensures businesses and individuals follow state tax laws, avoiding penalties and legal issues.",
    },
    {
        title: "Contributes to State Revenue",
        desc:
            "The tax collected supports local development and welfare programs, benefiting the community.",
    },
    {
        title: "Eligibility for Licence",
        desc:
            "Many businesses need professional tax registration to obtain necessary licenses and permits for operations.",
    },
    {
        title: "Improved Business Credibility",
        desc:
            "Demonstrates a business’s commitment to legal practices, which can boost its reputation.",
    },
    {
        title: "Tax Deductions",
        desc:
            "Employers can use professional tax payments as a deduction, potentially reducing taxable income.",
    },
];

function BenefitCard({ title, desc }: Benefit) {
    return (
        <article className="group relative h-full overflow-hidden rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200 transition">
            {/* Hover overlay (slides from top -> bottom) */}
            <div className="pointer-events-none absolute inset-0 translate-y-[-100%] bg-gradient-to-b from-[#142a63] to-[#0f265c] transition-transform duration-500 group-hover:translate-y-0" />

            {/* Content above overlay */}
            <div className="relative z-10 flex h-full flex-col">
                <h3 className="text-lg font-extrabold tracking-tight text-slate-800 transition-colors group-hover:text-white">
                    {title}
                </h3>
                <p className="mt-3 text-[15px] leading-7 text-slate-700 transition-colors group-hover:text-white/90">
                    {desc}
                </p>
            </div>
        </article>
    );
}

export default function BenefitsProfessionalTax() {
    return (
        <main className="bg-[#f6f8fc]">
            <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
                <h1 className="text-center text-[28px] sm:text-[34px] font-extrabold tracking-tight text-slate-900">
                    Benefits of Professional Tax Registration
                </h1>

                {/* Cards grid */}
                <div className="mx-auto mt-8 grid max-w-6xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {BENEFITS.map((b) => (
                        <div key={b.title} className="h-full">
                            <BenefitCard {...b} />
                        </div>
                    ))}
                </div>
            </section>
        </main>
    );
}
