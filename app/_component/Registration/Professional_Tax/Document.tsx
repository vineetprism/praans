"use client";

import React from "react";

/** Navy circular bullet */
const NavyBullet = () => (
  <svg viewBox="0 0 24 24" width={18} height={18} aria-hidden className="mt-1.5 shrink-0">
    <circle cx="12" cy="12" r="9" fill="none" stroke="#142a63" strokeWidth="2" />
    <circle cx="12" cy="12" r="3" fill="none" stroke="#142a63" strokeWidth="2" />
  </svg>
);

const LEFT_DOCS: string[] = [
  "Proof of Identity of the owner such as Aadhar card, Voter ID, PAN card, passport, etc.",
  "Certificate of Incorporation/Partnership Deed",
  "PAN card of the firm",
  "Article of Association",
];

const RIGHT_DOCS: string[] = [
  "GST certificate of the firm",
  "Proof of Establishment (rent agreement, utility bill, etc.)",
  "List of employees with name and gross salary",
];

export default function ProfessionalTaxDocs() {
  return (
    <main className="bg-[#f6f8fc] text-slate-900">
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <h2 className="text-center font-extrabold tracking-tight text-[#142a63] text-[24px] sm:text-[30px] lg:text-[34px] leading-snug">
          General Documents Required For Professional
          <br className="hidden sm:block" />
          <span className="block sm:inline"> Tax Registration In India</span>
        </h2>

        {/* One column by default; switch to two only when there's lots of width */}
        <div className="mx-auto mt-8 grid grid-cols-1 xl:grid-cols-2 gap-10 lg:gap-16 xl:gap-24 2xl:gap-40">
          {/* Left column */}
          <ul className="space-y-5 text-[15px] leading-7 text-slate-800">
            {LEFT_DOCS.map((item, idx) => (
              <li key={`l-${idx}`} className="flex items-start gap-3">
                <NavyBullet />
                {/* Allow wrapping on smaller screens; keep single-line only on very wide screens */}
                <span className="xl:whitespace-nowrap">{item}</span>
              </li>
            ))}
          </ul>

          {/* Right column */}
          <ul className="space-y-5 text-[15px] leading-7 text-slate-800">
            {RIGHT_DOCS.map((item, idx) => (
              <li key={`r-${idx}`} className="flex items-start gap-3">
                <NavyBullet />
                <span className="xl:whitespace-nowrap">{item}</span>
              </li>
            ))}
          </ul>
        </div>
      </section>
    </main>
  );
}
