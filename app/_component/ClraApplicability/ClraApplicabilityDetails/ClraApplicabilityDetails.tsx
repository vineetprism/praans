...
export default function LeavesWorkingHoursDetails({
  ...
  // First form URL
  const tileFormUrl = tiles?.form_path
    ? resolveFile(tiles?.form_path)
    : "";
  
  // Second form URL
  const tileFormUrl2 = tiles?.form_path2
    ? resolveFile(tiles?.form_path2)
    : "";
  ...
  return (
    <div className="min-h-screen w-full">
      <div className=" grid gap-3 sm:gap-4 lg:grid-cols-5  w-full max-w-none mx-auto px-2 py-2 sm:px-3 sm:py-3 md:px-4 md:py-4 lg:px-5 lg:py-4 xl:px-6 xl:py-5">
        {/* Popular Search (Mobile) */}
        <div className=" mb-3 sm:mb-4 w-full lg:col-span-1 lg:order-2 order-2">
        ...
        <div className="w-full grid gap-3 sm:gap-4 md:gap-5 lg:gap-4 xl:gap-5 lg:col-span-4 lg:order-1 order-1">
                  ...
                  {initialData?.act_information?.rows?.map((r, idx) => (
                    <div key={idx} className="rounded-lg p-2 sm:p-3 border text-xs sm:text-sm space-y-1 sm:space-y-2 w-full">
                      {initialData?.act_information?.headers?.map((h) => (
                        <div
                          key={h}
                          className="flex sm:flex-row justify-between border-b last:border-0 pb-1 gap-1 "
                        >
                          <div className="font-semibold text-orange-600 break-words">{h}:</div>
                          <div className="text-gray-800 text-justify ml-12">{r[h] ?? "—"}</div>
                        </div>
                      ))}
                    </div>
                  ))}
                  ...
                  <Card className="h-[140px] w-full max-w-[330px] text-center border-l-4 border-l-[#E85C0D] shadow-md hover:shadow-lg transition-shadow">
                    <CardContent className="h-full flex flex-col items-center justify-center">
                      ...
                      <h3 className="font-semibold mb-2 text-sm leading-tight text-[#222] px-2">
                        {tiles?.form_title ?? "Download Form"}
                      </h3>
                      <Button
                        size="sm"
                ...
                {tileFormUrl2 && (
                  <Card className="h-[140px] w-full max-w-[330px] text-center border-l-4 border-l-[#E85C0D] shadow-md hover:shadow-lg transition-shadow">
                    <CardContent className="h-full flex flex-col items-center justify-center">
                      <Download className="h-7 w-7 text-[#E85C0D] mb-2" />
                      <h3 className="font-semibold mb-2 text-sm leading-tight text-[#222] px-2">
                        {tiles?.form_title2 ?? "Download Notification"}
                      </h3>
                      <Button
                        size="sm"
                        className="w-[80%] h-9 bg-orange-200 hover:bg-[#d14e0b] text-orange-700 font-semibold text-sm truncate cursor-pointer rounded-md"
                        onClick={() => openProtectedDownload(router, tileFormUrl2)}
                      >
                        Download
                      </Button>
                    </CardContent>
