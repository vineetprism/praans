...
//           if (filtered[0]) {
//             setValue(filtered[0].label);
//             onSelect(filtered[0]); // auto-load for this state
//           }
//         }
//       } catch (e) {
//         console.error("[PeriodCombobox] options load failed:", e);
...
//                       setValue(opt.label);
//                       setOpen(false);
//                       onSelect(opt); // opt includes state info too
//                     }}
//                     className="cursor-pointer"
//                   >
//                     <Check
...
//   apiBase: string;
// }) {
//   // ✅ Local state ONLY for the first table (SSR seed)
//   const [mwTable, setMwTable] = React.useState<TableBlock>(() => data.minimum_wage_rates);
//   const [busy, setBusy] = React.useState(false);
...
//   // --- dropdown select handler: pass state_slug + dates ---
//   const handlePeriodSelect = React.useCallback(
//     async (opt: PeriodOption) => {
//       const uiStateSlug = (stateSlug || "").toLowerCase();
//       if (!uiStateSlug) return;

//       try {
//         setBusy(true);

//         const url = new URL("/api/minimum-wages/period", apiBase);
//         url.searchParams.set("state_slug", uiStateSlug);
//         // API expects dd-mm-YYYY (your sample)
//         url.searchParams.set("updated_on", opt.updated_date);
//         url.searchParams.set("effective_on", opt.effective_date);
...
//         // Safety: ensure server answered for the same state
//         const apiSlug = String(first?.state?.slug || "").toLowerCase();
//         if (!first || apiSlug !== uiStateSlug) {
//           console.warn("State mismatch or empty data", { uiStateSlug, apiSlug });
//           return;
//         }
...
// import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
// import { Button } from "@/components/ui/button";
// import { Download, Globe, IndianRupee, Check, ChevronsUpDown, Calendar, Loader2 } from "lucide-react";
// import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
// import { Command, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList } from "@/components/ui/command";
...
// // Build from origin to avoid base path issues
...
//     if (!apiBase) return null;
//     if (val.startsWith("/")) return new URL(val, API_ORIGIN).toString();
//     if (val.startsWith("storage/") || val.startsWith("/storage/")) return API_ORIGIN + (val.startsWith("/") ? "" : "/") + val;
//     return null;
//   }
// }
// function fileNameFromUrl(u: string): string {
...
//       try {
//         setLoading(true);
//         const url = apiPath(apiBase, "/api/minimum-wages/periods/options");
...
//   // Force re-mount + show active period
...
//   // period API → TableBlock  (✅ store value under BOTH label & key)
//   const toTableBlock = React.useCallback(
//     (periodRates: any): TableBlock => {
//       const cols = Array.isArray(periodRates?.columns) ? periodRates.columns : [];
//       const rows = Array.isArray(periodRates?.rows) ? periodRates.rows : [];

//       const headers = cols.map((c: any) => {
//         const label = String(c?.label ?? "").trim();
//         return label || String(c?.key ?? "").trim() || "—";
//       });
...
//           // dual mapping → robust against label/key/case changes
...
//   // select handler — pass state_slug + both dates
//   const handlePeriodSelect = React.useCallback(
//     async (opt: PeriodOption) => {
...
//         const urlStr = apiPath(apiBase, "/api/minimum-wages/period");
...
//         const apiSlug = String(first?.state?.slug || "").toLowerCase();
//         if (!first || apiSlug !== preferredSlug) {
//           console.warn("State mismatch/empty:", { asked: preferredSlug, got: apiSlug });
//           return;
//         }

//         const rates = first.minimum_wage_rates;
//         if (rates) {
//           setMwTable(toTableBlock(rates));      // new table data
//           setMwVersion((v) => v + 1);           // force re-mount
...
//             <div className="mb-4 sm:mb-5 lg:mb-4 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 sm:gap-3">
//               <div>
//                 <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-2xl xl:text-3xl font-bold text-slate-800">Minimum Wages of {data?.state?.name} :</h1>
//                 <div className="mt-1 inline-flex items-center gap-2 text-[11px] text-orange-700">
//                   <span className="px-2 py-0.5 rounded-full bg-orange-100 border border-orange-200">state_slug: {data?.state?.slug}</span>
//                   {busy && (
//                     <span className="inline-flex items-center gap-1">
//                       <Loader2 className="h-3.5 w-3.5 animate-spin" />
//                       updating…
...
//                 <PeriodCombobox apiBase={apiBase} stateSlug={data.state.slug} onSelect={handlePeriodSelect} defaultLabel="Select period" />
//                 {activePeriod && (
...
//                   <CardHeader className="pb-1 lg:pb-2">
//                     <CardTitle className="text-base lg:text-lg font-bold">{mwTable?.title || "Minimum Wage Rates (Daily in ₹)"}</CardTitle>
//                   </CardHeader>
//                   <CardContent className="p-0">
//                     <div className="w-full overflow-x-auto">
//                       <table className="w-full">
...
//                             <tr key={idx} className="hover:bg-orange-50 transition-colors">
//                               {mwTable?.headers?.map((h) => (
//                                 <td key={h} className={cn("px-3 py-4 text-sm text-gray-900 break-words text-center", isSkilledCol(h) && "text-green-600 font-semibold")}>
//                                   {fmt(cell(r, h))}
//                                 </td>
//                               ))}
//                             </tr>
...
//                     <CardHeader className="pb-2">
//                       <CardTitle className="text-base font-bold">{mwTable?.title || "Minimum Wage Rates (Daily in ₹)"}</CardTitle>
//                     </CardHeader>
//                     <CardContent className="p-3 space-y-3">
//                       {mwTable?.rows?.map((r, idx) => (
//                         <div key={idx} className="bg-gray-50 rounded-lg p-3 border text-xs space-y-2">
...
//                       {(mwTable?.rows?.length ?? 0) === 0 && <div className="text-center text-sm text-gray-500">No data available.</div>}
//                     </CardContent>
//                   </Card>
//                 </div>
//               </>
...
//             {/* ====== Table: Employment Categories (SSR) ====== */}
//             {cat && (
//               <>
//                 <Card className="hidden md:block mb-3 shadow-sm border-l-4 border-l-orange-500">
//                   <CardHeader className="pb-1 lg:pb-2">
...
//                   {formUrlNorm ? (
//                     <Button asChild size="sm" className="w-full bg-orange-500 hover:bg-orange-600 text-white text-xs lg:text-sm h-7 lg:h-8" aria-label={formBtnLabel}>
//                       <Link href={formUrlNorm} target="_blank" rel="noopener noreferrer" aria-label={formBtnLabel}>
//                         {formBtnLabel}
//                       </Link>
//                     </Button>
...
//                   <h3 className="font-medium mb-2 text-sm lg:text-base">Wage Calculator</h3>
//                   {calcUrlNorm ? (
//                     <Button asChild size="sm" variant="outline" className="w-full hover:bg-orange-50 hover:border-orange-200 hover:text-orange-600 text-xs lg:text-sm h-7 lg:h-8" aria-label="Open Calculator">
//                       <Link href={calcUrlNorm} target="_blank" rel="noopener noreferrer" aria-label="Open Calculator">
//                         Open Calculator
//                       </Link>
//                     </Button>
...
//                   <h3 className="font-medium mb-2 text-sm lg:text-base">Official Website</h3>
//                   {websiteUrlNorm ? (
//                     <Button asChild size="sm" variant="outline" className="w-full hover:bg-orange-50 hover:border-orange-200 hover:text-orange-600 text-xs lg:text-sm h-7 lg:h-8" aria-label="Visit Website">
//                       <Link href={websiteUrlNorm} target="_blank" rel="noopener noreferrer" aria-label="Visit Website">
//                         Visit Website
//                       </Link>
//                     </Button>
...
// /* ===== Types ===== */
// type Row = Record<string, string | null>;
// export type TableBlock = { title?: string | null; headers: string[]; rows: Row[]; };
// export type Tiles = { form_title?: string | null; form_url?: string | null; calculator_title?: string | null; calculator_url?: string | null; website_url?: string | null; };
// export type MWSlugData = {
//   state: { name: string; slug: string };
//   updated_date?: string | null;
//   effective_date?: string | null;
//   minimum_wage_rates: TableBlock;                // ← SSR me aayega, par FIRST TABLE me IGNORE karenge
//   employment_categories_benefits: TableBlock;    // ← SSR se render
//   interest_penality?: TableBlock;                // ← SSR se render
//   tiles?: Tiles;
// };
// type PeriodOption = {
//   id: number | string;
...
// const cell = (row: Row, header: string) => row?.[header] ?? "—";
// const fmt = (s?: string | null) => (s && s.trim() ? s : "—");
// const isSkilledCol = (h: string) => /skilled/i.test(h);
...
// /* ===== Dropdown (auto-select first option for this state) ===== */
// function PeriodCombobox({
//   apiBase, stateSlug, onSelect, defaultLabel,
// }: { apiBase: string; stateSlug: string; onSelect: (opt: PeriodOption) => void; defaultLabel?: string; }) {
//   const [open, setOpen] = React.useState(false);
//   const [loading, setLoading] = React.useState(true);
//   const [options, setOptions] = React.useState<PeriodOption[]>([]);
//   const [value, setValue] = React.useState<string>(defaultLabel || "Select period");
...
//       try {
//         setLoading(true);
//         const res = await fetch(apiPath(apiBase, "/api/minimum-wages/periods/options"), { cache: "no-store", headers: { Accept: "application/json" } });
//         const json = await res.json();
//         const all: PeriodOption[] = Array.isArray(json?.data) ? json.data : [];
...
//         // 🔥 Auto-select first option for this state
//         if (filtered[0]) {
//           setValue(filtered[0].label);
//           onSelect(filtered[0]);
//         }
...
// export default function MinimumWageDetails({ data, apiBase }: { data: MWSlugData; apiBase: string; }) {
//   /** ✅ IMPORTANT:
//    *  First table ko SSR se IGNORE kar rahe hain. Isliye initial mwTable = null.
//    *  Period dropdown auto-select → period API hit → mwTable hydrate.
//    */
...
//   // columns/rows → TableBlock  (dual-map: label + key)
//   const toTableBlock = React.useCallback((periodRates: any): TableBlock => {
//     const cols = Array.isArray(periodRates?.columns) ? periodRates.columns : [];
//     const rows = Array.isArray(periodRates?.rows) ? periodRates.rows : [];

//     const headers = cols.map((c: any) => (String(c?.label ?? "").trim() || String(c?.key ?? "").trim() || "—"));
//     const mappedRows: Row[] = rows.map((r: any) => {
//       const obj: Row = {};
//       cols.forEach((c: any) => {
//         const key = String(c?.key ?? "").trim();
...
//   // dropdown select → period API call (state_slug + dates)
...
//     try {
//       setBusy(true);
//       const url = new URL(apiPath(apiBase, "/api/minimum-wages/period"));
//       url.searchParams.set("state_slug", preferredSlug);
//       url.searchParams.set("updated_on",  opt.updated_date);     // dd-mm-YYYY
//       url.searchParams.set("effective_on", opt.effective_date);  // dd-mm-YYYY
...
//       const json = await res.json();
//       const first = Array.isArray(json?.data) ? json.data[0] : null;
//       if (!first || String(first?.state?.slug || "").toLowerCase() !== preferredSlug) return;

//       const rates = first.minimum_wage_rates;
//       if (rates) {
//         setMwTable(toTableBlock(rates));   // hydrate first table
//         setMwVersion(v => v + 1);          // force re-mount
//         setActivePeriod({ updated_on: first.updated_date, effective_on: first.effective_date });
//       }
//     } catch (e) {
...
//   // tiles normalized
//   const formUrl = normalizeUrl(tiles.form_url, apiBase);
//   const calcUrl = normalizeUrl(tiles.calculator_url, apiBase);
//   const siteUrl = normalizeUrl(tiles.website_url, apiBase);
//   const formTitle = fmt(tiles.form_title);
...
//                 </h1>
//                 <div className="mt-1 inline-flex items-center gap-2 text-[11px] text-orange-700">
//                   <span className="px-2 py-0.5 rounded-full bg-orange-100 border border-orange-200">state_slug: {stateSlug}</span>
//                   {busy && <span className="inline-flex items-center gap-1"><Loader2 className="h-3.5 w-3.5 animate-spin" /> updating…</span>}
//                 </div>
//               </div>
//               <div className="flex flex-col items-start sm:items-end gap-1.5">
//                 <PeriodCombobox apiBase={apiBase} stateSlug={stateSlug!} onSelect={handlePeriodSelect} defaultLabel="Select period" />
//                 {activePeriod && (
//                   <div className="text-[11px] text-orange-700">
//                     Updated <b>{activePeriod.updated_on}</b> • Effective <b>{activePeriod.effective_on}</b>
//                   </div>
...
//             {!mwTable ? (
//               // loader state till first period fetch completes
//               <Card key={`mw-loading`} className="mb-3 shadow-sm border-l-4 border-l-orange-500">
//                 <CardHeader className="pb-1 lg:pb-2">
...
//                           <tr className="bg-orange-500">
//                             {mwTable.headers.map((h) => (
//                               <th key={h} className="px-3 py-3 text-sm font-bold text-white uppercase tracking-wide text-center">{h}</th>
//                             ))}
//                           </tr>
//                         </thead>
//                         <tbody className="bg-white divide-y divide-gray-200">
...
//                             <tr><td colSpan={mwTable.headers.length || 1} className="px-4 py-6 text-center text-sm text-gray-500">No data available.</td></tr>
//                           )}
//                         </tbody>
//                       </table>
//                     </div>
...
//                 <div className="block md:hidden mb-4">
//                   <Card key={`mw-m-${mwVersion}`} className="shadow-sm border-l-4 border-l-orange-500">
//                     <CardHeader className="pb-2"><CardTitle className="text-base font-bold">{mwTable?.title || "Minimum Wage Rates (Daily in ₹)"}</CardTitle></CardHeader>
//                     <CardContent className="p-3 space-y-3">
//                       {mwTable.rows.map((r, idx) => (
//                         <div key={idx} className="bg-gray-50 rounded-lg p-3 border text-xs space-y-2">
//                           {mwTable.headers.map((h) => (
...
//                         </div>
//                       ))}
//                       {mwTable.rows.length === 0 && <div className="text-center text-sm text-gray-500">No data available.</div>}
//                     </CardContent>
//                   </Card>
//                 </div>
//               </>
...
//             {/* ===== Employment Categories (SSR) ===== */}
//             {cat && (
//               <>
//                 <Card className="hidden md:block mb-3 shadow-sm border-l-4 border-l-orange-500">
//                   <CardHeader className="pb-1 lg:pb-2"><CardTitle className="text-base lg:text-lg font-bold">Employment Categories &amp; Additional Benefits</CardTitle></CardHeader>
//                   <CardContent className="p-0">
//                     <div className="w-full overflow-x-auto">
//                       <table className="w-full">
//                         <thead><tr className="bg-orange-500">{cat.headers.map(h => <th key={h} className="px-3 py-3 text-sm font-bold text-white uppercase tracking-wide text-center">{h}</th>)}</tr></thead>
//                         <tbody className="bg-white divide-y divide-gray-200">
//                           {cat.rows.map((r, idx) => (
//                             <tr key={idx} className="hover:bg-orange-50 transition-colors">
//                               {cat.headers.map(h => <td key={h} className="px-3 py-4 text-sm text-gray-900 break-words text-center">{fmt(cell(r, h))}</td>)}
//                             </tr>
//                           ))}
//                           {cat.rows.length === 0 && <tr><td colSpan={cat.headers.length || 1} className="px-4 py-6 text-center text-sm text-gray-500">No data available.</td></tr>}
//                         </tbody>
//                       </table>
//                     </div>
//                   </CardContent>
...
//                 <div className="block md:hidden mb-4">
//                   <Card className="shadow-sm border-l-4 border-l-orange-500">
//                     <CardHeader className="pb-2"><CardTitle className="text-base font-bold">Employment Categories &amp; Additional Benefits</CardTitle></CardHeader>
//                     <CardContent className="p-3 space-y-3">
//                       {cat.rows.map((r, idx) => (
//                         <div key={idx} className="bg-gray-50 rounded-lg p-3 border text-xs space-y-2">
//                           {cat.headers.map(h => (
//                             <div key={h} className="flex gap-2 justify-between p-2 rounded">
//                               <span className="font-medium text-gray-600">{h}:</span><span>{fmt(cell(r, h))}</span>
//                             </div>
//                           ))}
//                         </div>
//                       ))}
//                       {cat.rows.length === 0 && <div className="text-center text-sm text-gray-500">No data available.</div>}
//                     </CardContent>
//                   </Card>
//                 </div>
//               </>
...
//             {/* ===== Interest & Penalty (SSR) ===== */}
//             {ip && (
//               <>
//                 <Card className="hidden md:block mb-3 shadow-sm border-l-4 border-l-orange-500">
//                   <CardHeader className="pb-1 lg:pb-2"><CardTitle className="text-base lg:text-lg font-bold">{ip.title || "Interest & Penalty"}</CardTitle></CardHeader>
//                   <CardContent className="p-0">
//                     <div className="w-full overflow-x-auto">
//                       <table className="w-full">
//                         <thead><tr className="bg-orange-500">{ip.headers.map(h => <th key={h} className="px-3 py-3 text-sm font-bold text-white uppercase tracking-wide text-center">{h}</th>)}</tr></thead>
//                         <tbody className="bg-white divide-y divide-gray-200">
//                           {ip.rows.map((r, idx) => (
//                             <tr key={idx} className="hover:bg-orange-50 transition-colors">
//                               {ip.headers.map(h => <td key={h} className="px-3 py-4 text-sm text-gray-900 break-words text-center">{fmt(cell(r, h))}</td>)}
//                             </tr>
//                           ))}
//                           {ip.rows.length === 0 && <tr><td colSpan={ip.headers.length || 1} className="px-4 py-6 text-center text-sm text-gray-500">No data available.</td></tr>}
//                         </tbody>
//                       </table>
//                     </div>
//                   </CardContent>
...
//                 <div className="block md:hidden mb-4">
//                   <Card className="shadow-sm border-l-4 border-l-orange-500">
//                     <CardHeader className="pb-2"><CardTitle className="text-base font-bold">{ip.title || "Interest & Penalty"}</CardTitle></CardHeader>
//                     <CardContent className="p-3 space-y-3">
//                       {ip.rows.map((r, idx) => (
//                         <div key={idx} className="bg-gray-50 rounded-lg p-3 border text-xs space-y-2">
//                           {ip.headers.map(h => (
...
//                         </div>
//                       ))}
//                       {ip.rows.length === 0 && <div className="text-center text-sm text-gray-500">No data available.</div>}
//                     </CardContent>
//                   </Card>
//                 </div>
//               </>
...
//                   {formUrl ? (
//                     <Button asChild size="sm" className="w-full bg-orange-500 hover:bg-orange-600 text-white text-xs lg:text-sm h-7 lg:h-8" aria-label={formBtnLabel}>
//                       <Link href={formUrl} target="_blank" rel="noopener noreferrer">{formBtnLabel}</Link>
//                     </Button>
//                   ) : (
//                     <Button size="sm" disabled className="w-full text-xs lg:text-sm h-7 lg:h-8">No Form Available</Button>
//                   )}
//                 </CardContent>
//               </Card>
...
//                   <h3 className="font-medium mb-2 text-sm lg:text-base">Wage Calculator</h3>
//                   {calcUrl ? (
//                     <Button asChild size="sm" variant="outline" className="w-full hover:bg-orange-50 hover:border-orange-200 hover:text-orange-600 text-xs lg:text-sm h-7 lg:h-8"><Link href={calcUrl} target="_blank" rel="noopener noreferrer">Open Calculator</Link></Button>
//                   ) : (
//                     <Button size="sm" variant="outline" disabled className="w-full text-xs lg:text-sm h-7 lg:h-8">Not Available</Button>
//                   )}
//                 </CardContent>
//               </Card>
...
//                   <h3 className="font-medium mb-2 text-sm lg:text-base">Official Website</h3>
//                   {siteUrl ? (
//                     <Button asChild size="sm" variant="outline" className="w-full hover:bg-orange-50 hover:border-orange-200 hover:text-orange-600 text-xs lg:text-sm h-7 lg:h-8"><Link href={siteUrl} target="_blank" rel="noopener noreferrer">Visit Website</Link></Button>
//                   ) : (
//                     <Button size="sm" variant="outline" disabled className="w-full text-xs lg:text-sm h-7 lg:h-8">Not Available</Button>
//                   )}
//                 </CardContent>
...
//           <div className="hidden lg:block lg:col-span-1">
//             <div className="sticky top-2 lg:top-3">
//               <Card className="shadow-sm hover:shadow-md transition-shadow"><CardContent className="lg:p-3 xl:p-4"><PopularSearch className="mb-0" /></CardContent></Card>
//             </div>
//           </div>
//         </div>
//       </div>
