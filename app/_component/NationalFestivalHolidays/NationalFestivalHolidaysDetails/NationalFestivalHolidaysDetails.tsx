...
const gridColsClass = (count: number) => {
  // Tailwind needs explicit classes; map to the nearest up-to-4 layout
...
const badgeTone = (value: string) => {
  ...
  if (["yes", "y", "1", "true"].includes(normalized)) return "bg-green-100 text-green-800";
  if (["no", "n", "0", "false"].includes(normalized)) return "bg-red-100 text-red-800";
  return "bg-gray-100 text-gray-800";
};

/**
 * Robust extractor for `Applicability to` like: "Factories(Yes) Shops(No) ..."
 */
const parseApplicability = (value: string) => {
  const categories: Record<string, string> = {};
  const regex = /(\S+?)\s*\(([^)]+)\)/g; // tolerant of spaces
...
export default function NationalFestivalHolidaysDetails({ initialData }: Props) {
  ...
  const handleDownload = () => {
    if (stateData.tiles?.form_url) window.open(stateData.tiles.form_url, "_blank");
  };
  ...
  const renderCellContent = (header: string, rawValue: unknown) => {
        ...
        <div className={`grid ${cols} gap-3 sm:gap-4`}> 
          {keys.map((k) => (
            <div key={k} className="min-w-0">
              <div className="font-medium text-gray-900 capitalize truncate">{k.replaceAll("_", " ")}</div>
              <Badge className={`mt-1 ${badgeTone(categories[k])}`}>{categories[k]}</Badge>
            </div>
          ))}
        </div>
      );
    ...
    if (header === "List of National Holidays") {
      ...
      else if (value.includes(",")) holidays = value.split(",").map((x) => x.trim()).filter(Boolean);
      else if (value.includes("\n")) holidays = value.split("\n").map((x) => x.trim()).filter(Boolean);
      else holidays = value.split(/\s+/).filter((x) => x.includes("("));
      const count = holidays.length
      return (
        <div className="flex items-start gap-3 sm:gap-4 flex-wrap">
          {/* <Badge className="font-semibold bg-blue-100 text-blue-800">{count}</Badge> */}
          <ul className="list-disc list-inside space-y-1 text-gray-700 w-full sm:w-auto">
            {holidays.length ? holidays.map((h, i) => <li key={i}>{h}</li>) : <li>{value}</li>}
          </ul>
        </div>
      );
    }
    ...
    if (header === "Total Number of National & Festival Holidays in a year") {
      return <Badge className="bg-purple-100 text-purple-800 font-semibold text-base sm:text-lg px-2.5 py-1">{value}</Badge>;
    }

    return <span className="text-gray-700 break-words whitespace-pre-wrap">{value}</span>;
  };
        ...
        <div className="mb-4 sm:mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div className="sm:shrink-0">
              <Button onClick={handleDownload} className="bg-orange-500 hover:bg-orange-600 text-white w-full sm:w-auto">
          ...
          <div className="lg:col-span-3">
            ...
            <Card className="border-l-4 border-l-orange-500">
              <CardHeader className="bg-gray-50 border-b border-gray-200">
                <CardTitle className="text-xl sm:text-2xl text-gray-900">National Festival Holiday Matrix</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                {/* Mobile / Tablet: stacked definition-style */}
                <div className="lg:hidden divide-y">
                  {headers.map((header) => (
                    <div key={header} className="p-4">
                      <div className="text-sm font-semibold text-gray-900 mb-2">{header}</div>
                      <div className="space-y-2">
                        {rows.map((row, idx) => (
                          <div key={idx}>{renderCellContent(header, (row as any)[header])}</div>
                        ))}
                      </div>
                    </div>
                  ))}
                ...
                <div className="hidden lg:block overflow-x-auto">
                  <table className="w-full text-left">
                    <tbody>
                      {headers.map((header) => (
                        <tr key={header} className="border-b border-gray-200 align-top">
                          <td className="p-4 lg:p-5 font-semibold text-gray-900 bg-gray-50 w-1/3 min-w-[260px]">{header}</td>
