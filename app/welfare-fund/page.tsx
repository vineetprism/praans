...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Button } from "@/components/ui/button";
import { Search, CheckCircle, XCircle, Calculator, CalculatorIcon, Eye, MapPin } from "lucide-react";
import PopularSearch from "../PopularSearch/PopularSearch";
...
export default function WelfareFundPage() {
  ...
  // Build dropdown options from API
  const stateOptions = useMemo(() => {
    const names = new Set<string>();
    (data?.applicable_states ?? []).forEach((s) => names.add(s.state_name));
    (data?.non_applicable_states ?? []).forEach((s) => names.add(s.state_name));
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-gray-600 text-xs sm:text-sm md:text-base lg:text-[0.9rem] text-justify leading-relaxed">
                      Labour welfare provides facilities to workers to improve their working conditions, social security, and
                      standard of living. Many states have enacted a Labour Welfare Fund Act governing contributions and
                      benefits.
                    </p>
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center mb-6 sm:mb-8 md:mb-10">
                ...
                <div className="w-full sm:w-auto">
                  <Select value={stateFilter} onValueChange={setStateFilter}>
                    <SelectTrigger
                      className="w-full sm:w-48 md:w-52 h-8 sm:h-9 md:h-10 bg-white text-gray-900 border border-gray-300 rounded-lg text-xs sm:text-sm
