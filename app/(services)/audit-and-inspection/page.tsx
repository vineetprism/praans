...
function resolveCanonical(metaUrl?: string | null): string {
  if (!metaUrl) return `${API_BASE}/service/audit`;
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const res = await fetch(`${API_BASE}/api/service/audit`, {
      next: { revalidate },
    });
    ...
    if (!res.ok) {
      return {
        title:
          "Audit & Inspection Services in Gurugram | Praans Consultech",
        description:
          "End-to-end Audit & Inspection services for contractors and principal employers. Fast TAT, expert guidance, PAN-India support, and free consultation.",
        keywords: [
          "Audit & Inspection",
        ...
        alternates: { canonical: `${API_BASE}/service/audit` },
      };
    }
