...
function resolveCanonical(metaUrl?: string | null): string {
  if (!metaUrl) return `${API_BASE}/service/compliance`;
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const res = await fetch(`${API_BASE}/api/service/compliance`, {
      next: { revalidate },
    });
    ...
    if (!res.ok) {
      return {
        title:
          "Compliance Outsourcing Services in Gurugram | Praans Consultech",
        description:
          "End-to-end Compliance Outsourcing services for contractors and principal employers. Fast TAT, expert guidance, PAN-India support, and free consultation.",
        keywords: [
        ...
        alternates: { canonical: `${API_BASE}/service/compliance` },
      };
    }
