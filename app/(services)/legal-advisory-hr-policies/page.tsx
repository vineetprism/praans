...
function resolveCanonical(metaUrl?: string | null): string {
  if (!metaUrl) return `${API_BASE}/service/legal`;
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const res = await fetch(`${API_BASE}/api/service/legal`, {
      next: { revalidate },
    });
    ...
    if (!res.ok) {
      return {
        title:
          "Legal Advisory HR Policies Services in Gurugram | Praans Consultech",
        description:
          "End-to-end Legal Advisory HR Policies services for contractors and principal employers. Fast TAT, expert guidance, PAN-India support, and free consultation.",
        keywords: [
          "Legal Advisory HR Policies",
        ...
        alternates: { canonical: `${API_BASE}/service/legal` },
      };
    }
