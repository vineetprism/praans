...
function resolveCanonical(metaUrl?: string | null): string {
  if (!metaUrl) return `${API_BASE}/service/pan`;
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const res = await fetch(`${API_BASE}/api/service/pan`, {
      next: { revalidate },
    });
    ...
    if (!res.ok) {
      return {
        title:
          "Pan India Registrations Services in Gurugram | Praans Consultech",
        description:
          "End-to-end Pan India Registrations services for contractors and principal employers. Fast TAT, expert guidance, PAN-India support, and free consultation.",
        keywords: [
          "Pan India Registrations",
        ...
        alternates: { canonical: `${API_BASE}/service/pan` },
      };
    }
