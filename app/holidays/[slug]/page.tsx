...
export const revalidate = 1800; // 30m ISR for whole route
...
async function getHolidayState(slug: string): Promise<HolidayStateData | null> {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/holidaysdetail/${slug}`, {
      next: { revalidate }, // use route-level ISR
    });
...
// (optional) dynamic metadata — safe fallback if API fails
export async function generateMetadata({
  params,
}: { params: { slug: string } }): Promise<Metadata> {
  const data = await getHolidayState(params.slug);
  const title =
    data ? `${data.title} Holidays ${data.year} | Prism PRNS` : "Holiday Details | Prism PRNS";
  const description =
    data?.short_desc
