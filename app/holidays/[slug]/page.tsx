...
import Link from "next/link";
import { useState } from "react";
import { Calendar, MapPin, ArrowLeft, Download, CalendarDays } from "lucide-react";
import PopularSearch from "@/app/PopularSearch/PopularSearch";
...
const holidayData = [
  {
    ...
    type: "Regional",
    remarks: "NULL",
    fullDate: new Date(2025, 0, 14), // JavaScript months are 0-indexed
  },
  {
    id: 2,
    name: "Republic day",
...
const DatePicker = ({ selected, onChange, placeholder }) => {
  return (
    <input
      type="date"
...
export default function StateDetailPage() {
  ...
  const filteredHolidays = holidayData.filter((holiday) => {
    ...
    if (startDate && endDate) {
      const holidayDate = holiday.fullDate;
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 py-5">
                    ...
                    {/* <div className="flex items-end">
                      <Button
                        ...
                        className="w-full h-7 min-[375px]:h-8 sm:h-9 md:h-10 text-xs sm:text-sm hover:bg-orange-50 hover:border-orange-200 hover:text-orange-600"
                      >
